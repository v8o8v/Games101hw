...
{   
    ...
    ap << point-_v[0].head(2);
    bp << point-_v[1].head(2);
    cp << point-_v[2].head(2);
    ...
    float abap= ab.x() * ap.y() - ap.x() * ab.y();
    float bcbp= bc.x() *bp.y() - bp.x() * bc.y();
    float cacp= ca.x() *cp.y() - cp.x() * ca.y();
    ...
    for(float x=minx ; x<maxx ; x++)
    {
        for(float y=miny ; y<maxy ; y++)
        {
            ...
            if(super_sampling)
            {
                ...
                if(count>0 && mindep < depth_buf[get_index(x,y)])
                {
                    depth_buf[get_index(x,y)]=mindep;
                    set_pixel(Vector3f(x,y,mindep),t.getColor()*count);
                }
                count=0;
                mindep=100.0f;
            }
